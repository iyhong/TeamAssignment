...
public class Grammer07_myungchul {
	public static void main(String[] args) {
		...
		기본형 타입 (Primitive Type):데이터의 기본 유형이며 byte, short, int, long, float, double, boolean, char 변수에 값을 저장한다.
		int i = 1;
		...
		*/
		...
		//String클래스인 s1 변수를 선언하고  String 생성자 객체를 만드고, 객체의 데이터영역에 주소값과 문자열인 "one"를 s1변수에 대입한다.
		//마찬가지로 s2변수에 "one"를 대입한다.
		//equals()메서드는 객체의 주소값이 아닌 내용을 비교하기 떄문에 s1와s2가 문자열의 내용이 같이 떄문에 true로 리턴한다.
		...
		4번이 true가 되는 경우는 GregorianCalendar 클래스를 지정한 날짜에서 getTime메서드는 시간값의 밀리초 단위로 가져와서 사용하여 시간을 더한다.
		d2나 d3는 지정한 날짜와 시간이 고정되어 있기 때문에 true로 리턴하게 된다.
		5번이 false로 리턴되는 경우는 d1과 d4의  Date() 메서드에 날짜가 같다. 단, 가져온 시간값이 다르다.
		d1가 실행되고 나서 d2 / d3가 실행되고나서 d4가 실행이 되는데 시간이 다른게 입력이 된다.
		Date 개체에는 밀리초 단위까지 특정 인스턴스를 시간으로 나타내는 숫자가 포함되어 있습니다
		d1에 시간이 11:53:01 21밀리초 라면 d4에는 11:53:01 45밀리초 입력이 되고 equals()메서드에서 내용을 비교 하기 되면 밀리초의 값이 달라서 false로 리턴이 된다. 
		*/
